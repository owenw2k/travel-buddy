...
- Next.js (App Router), Tailwind, shadcn/ui, Zustand, react-simple-maps, idb-keyval
...
- Persistence helpers mock idb-keyval — never touch real IndexedDB in unit tests
...
- Inline comments explain the _why_: map projection quirks, IndexedDB async patterns, URL encoding for share links
...
- **Chromium-only** — detect non-Chromium via `src/lib/browser.ts`, render `<NonChromiumFallback />` (`src/components/NonChromiumFallback.tsx`) before any map loads — same playful error page theme as 404/500 pages
...
- Map state (regions, legends, world toggle) persisted to IndexedDB via idb-keyval — do not use localStorage for map state
